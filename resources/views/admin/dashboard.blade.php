...
<div class="space-y-6 pb-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        --}}
        <a href="{{ route('admin.teachers.index') }}" class="block card transition-colors hover:border-indigo-300">
            <p class="text-xs font-semibold text-slate-500">Wali Kelas Terdaftar</p>
            <p class="mt-1 text-3xl font-semibold text-slate-900">{{ number_format($ringkas['guru_aktif']) }}</p>
            ...
            <p class="mt-2 text-[11px] font-semibold text-indigo-600">Lihat daftarnya ›</p>
        </a>
    ...
    <!-- Segmen langganan -->
    <div class="card">
        <h3 class="text-sm font-semibold text-slate-900">Segmen Langganan</h3>
        ...
        <div class="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-3">
            @foreach([
                ...
                <div class="rounded border p-4 {{ $kotak }}">
                    <p class="text-2xl font-semibold {{ $angka }}">{{ number_format($jumlah) }}</p>
                    <p class="eyebrow mt-0.5 {{ $judul }}">{{ $label }}</p>
    ...
    <div class="card space-y-4">
        <div class="flex items-center justify-between border-b border-slate-200 pb-3">
            <div>
                <h3 class="text-sm font-semibold text-slate-900">Masa Aktif Habis / Segera Habis ({{ $perluDitagih->count() }})</h3>
                ...
                <table class="w-full text-left text-xs">
                    <thead class="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 font-semibold border-b border-slate-200">
                        <tr>
                            ...
                            <th scope="col" class="px-3 py-2">Berakhir</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @foreach($perluDitagih as $g)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-3 py-2">
                                    <span class="inline-block rounded-md px-2 py-0.5 text-[10px] font-semibold {{ $g['pernah_bayar'] ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700' }}">
                                ...
                                <td class="px-3 py-2">
                                    @if($g['sisa_hari'] < 0)
                                        <span class="font-semibold text-rose-600">habis {{ abs($g['sisa_hari']) }} hari lalu</span>
                                    @elseif($g['sisa_hari'] <= 3)
                                        <span class="font-semibold text-amber-600">sisa {{ $g['sisa_hari'] }} hari</span>
                                    @else
        ...
        @else
            <p class="rounded bg-slate-50 py-6 text-center text-xs text-slate-500">Tidak ada masa aktif yang jatuh tempo dalam rentang ini.</p>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="card space-y-4">
            <div class="border-b border-slate-200 pb-3">
                <h3 class="text-sm font-semibold text-slate-900">Kesehatan Gateway WhatsApp</h3>
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($gateway['per_status'] as $status => $jumlah)
                    <span class="rounded-sm px-2.5 py-0.5 text-[10px] font-semibold {{ match($status) { 'connected' => 'bg-emerald-100 text-emerald-800', 'pairing' => 'bg-amber-100 text-amber-800', default => 'bg-slate-200 text-slate-700', } }}">
                    ...
                    @foreach($gateway['bermasalah'] as $g)
                        <li class="py-2.5">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <span class="text-xs font-semibold text-slate-900 block truncate">{{ $g['nama'] }}</span>
                                ...
                                <span class="shrink-0 rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-700">{{ $g['status'] }}</span>
                            </div>
                        </li>
                    @endforeach
                </ul>
            @else
                <p class="rounded bg-emerald-50 py-6 text-center text-xs font-semibold text-emerald-700">Semua sesi tersambung.</p>
            @endif
        </div>
        ...
        <div class="card space-y-4">
            <div class="border-b border-slate-200 pb-3">
                <h3 class="text-sm font-semibold text-slate-900">Antrian &amp; Pekerjaan Gagal</h3>
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="rounded bg-slate-50 p-3">
                    ...
                    <p class="text-xl font-semibold text-slate-900">{{ $antrian['menunggu'] ?? '—' }}</p>
                </div>
                <div class="rounded {{ $antrian['gagal_24j'] > 0 ? 'bg-rose-50' : 'bg-slate-50' }} p-3">
                    <p class="eyebrow {{ $antrian['gagal_24j'] > 0 ? 'text-rose-700' : 'text-slate-500' }}">Gagal 24 jam</p>
                    <p class="text-xl font-semibold {{ $antrian['gagal_24j'] > 0 ? 'text-rose-900' : 'text-slate-900' }}">{{ $antrian['gagal_24j'] }}</p>
                </div>
                <div class="rounded bg-slate-50 p-3">
                    <p class="eyebrow">Gagal total</p>
                    <p class="text-xl font-semibold text-slate-900">{{ $antrian['gagal_total'] }}</p>
                </div>
            </div>

            @if($antrian['redis_galat'])
                <div class="rounded border border-rose-200 bg-rose-50 p-3">
                    <p class="text-xs font-semibold text-rose-800">Redis tidak terbaca — antrian mungkin tidak jalan</p>
                    ...
                    @foreach($antrian['gagal_terbaru'] as $j)
                        <li class="py-2.5">
                            <div class="flex items-baseline justify-between gap-3">
                                <span class="text-xs font-semibold text-slate-900 truncate">{{ class_basename($j['job']) }}</span>
            ...
            @else
                <p class="rounded bg-emerald-50 py-6 text-center text-xs font-semibold text-emerald-700">Tidak ada pekerjaan gagal.</p>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-2 card">
            <div class="flex items-baseline justify-between">
                <div>
                    <h3 class="text-sm font-semibold text-slate-900">Pendaftar per Pekan</h3>
            ...
            <div class="mt-5 flex items-end justify-between gap-2 h-32">
                @foreach($pertumbuhan['mingguan'] as $pekan)
                    <div class="flex flex-1 flex-col items-center justify-end gap-1.5 h-full">
                        <span class="text-[10px] font-semibold text-slate-600">{{ $pekan['jumlah'] }}</span>
        ...
        <div class="card">
            <h3 class="text-sm font-semibold text-slate-900">Skala Platform</h3>
            ...
            <dl class="mt-4 space-y-3">
                @foreach([
                    ...
                    <div class="flex items-baseline justify-between border-b border-slate-200 pb-2 last:border-0">
                        <dt class="text-xs text-slate-500">{{ $label }}</dt>
                        <dd class="text-lg font-semibold text-slate-900">{{ number_format($nilai) }}</dd>
                    </div>
                @endforeach
            </dl>
        </div>
