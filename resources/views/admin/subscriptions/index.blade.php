...
<div class="space-y-6 pb-12">
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <span class="rounded-full bg-amber-100 px-2.5 py-0.5 text-[10px] font-bold text-amber-800">
                Pending Approval
            </span>
        </div>
            ...
            <div class="overflow-x-auto rounded-xl border border-slate-200">
                <table class="w-full text-left text-xs">
                    <thead class="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 font-bold border-b border-slate-200">
                        <tr>
                            <th scope="col" class="py-3 px-3">Guru &amp; Sekolah</th>
                            <th scope="col" class="py-3 px-3">Paket</th>
                            <th scope="col" class="py-3 px-3">Pengirim / Bank</th>
                            <th scope="col" class="py-3 px-3 text-center">Bukti Transfer</th>
                            <th scope="col" class="py-3 px-3 text-center">Aksi Konfirmasi</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($pending as $p)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="py-3 px-3 text-center">
                                    <a href="{{ route('subscription.proof', $p) }}" target="_blank" class="inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:underline">
                                        🖼️ Lihat Foto Bukti
                                    </a>
                                </td>
                                <td class="py-3 px-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.subscriptions.reject', $p) }}">
                                            @csrf
                                            <button type="submit" onclick="return confirm('Tolak bukti pembayaran ini?')" class="h-8 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 px-2.5 font-bold text-xs transition-colors">
                                                ✕ Tolak
                                            </button>
                                        </form>
                                    </div>
                ...
                <table class="w-full text-left text-xs">
                    <thead class="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 font-bold border-b border-slate-200">
                        <tr>
                            ...
                            <th scope="col" class="py-3 px-3">Guru</th>
                            <th scope="col" class="py-3 px-3">Paket</th>
                            <th scope="col" class="py-3 px-3 text-center">Status</th>
                            <th scope="col" class="py-3 px-3">Diproses Oleh</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @foreach($history as $h)
                            <tr class="hover:bg-slate-50/80">
                                <td class="py-3 px-3 text-slate-500">{{ $h->created_at->format('d M Y H:i') }}</td>
                                <td class="py-3 px-3 font-bold text-slate-900">{{ $h->user->name ?? '-' }}</td>
                                ...
                                <td class="py-3 px-3 text-center">
                                    <span class="inline-block rounded-md px-2 py-0.5 text-[10px] font-bold {{ $h->status === 'approved' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">
                                        {{ ucfirst($h->status) }}
                                    </span>
                                </td>
                                <td class="py-3 px-3 text-slate-500">{{ $h->approver->name ?? '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
