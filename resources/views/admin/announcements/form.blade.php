...
<div class="max-w-xl space-y-6 pb-12">
    <div>
        ...
        <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">Pengumuman ke Semua Guru</h1>
    ...
    <form method="POST" action="{{ route('admin.announcements.send') }}"
          ...
          class="space-y-4 rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        @csrf
        <div>
            <label for="title" class="block text-xs font-bold text-slate-700">Judul</label>
        ...
        <div>
            <label for="body" class="block text-xs font-bold text-slate-700">Isi</label>
            <textarea id="body" name="body" rows="5" maxlength="2000" required
                      class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-amber-500 focus:ring-amber-500"
                      placeholder="Tulis pengumuman...">{{ old('body') }}</textarea>
        </div>
        <button type="submit" class="rounded-lg bg-amber-500 px-4 py-2 text-sm font-bold text-slate-900 hover:bg-amber-600">
