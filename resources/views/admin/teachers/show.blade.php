...
<div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div class="min-w-0">
            <a href="{{ route('admin.teachers.index') }}" class="text-xs font-semibold text-indigo-600 hover:text-indigo-700">‹ Daftar Guru</a>
        ...
        <div class="text-xs text-slate-500 sm:text-right shrink-0 space-y-2">
            ...
            <form method="POST" action="{{ route('admin.teachers.toggle-active', $guru) }}"
                ...
                @csrf
                @if ($guru->is_active)
                    <button type="submit" class="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-rose-700">Nonaktifkan akun</button>
                @else
                    <button type="submit" class="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700">Aktifkan kembali</button>
                @endif
            </form>
            ...
            <form method="POST" action="{{ route('admin.teachers.reset-password', $guru) }}"
                ...
                @csrf
                <button type="submit" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Reset kata sandi</button>
            </form>
            ...
            @if ($guru->is_active)
                <form method="POST" action="{{ route('admin.teachers.impersonate', $guru) }}"
                    ...
                    @csrf
                    <button type="submit" class="rounded-lg border border-indigo-300 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-50">Masuk sebagai guru</button>
                </form>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="card">
            ...
            <p class="mt-1 text-lg font-semibold {{ $pro ? 'text-amber-700' : 'text-slate-900' }}">{{ $pro ? 'PRO' : 'Gratis' }}</p>
            ...
            <form method="POST" action="{{ route('admin.teachers.grant-pro', $guru) }}" class="mt-3 flex items-center gap-2">
                ...
                <button type="submit"
                        class="rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-semibold text-slate-900 hover:bg-amber-600">Beri PRO</button>
            </form>
        </div>

        <div class="card">
            <p class="text-xs font-semibold text-slate-500">Gateway WhatsApp</p>
            <p class="mt-1 text-lg font-semibold {{ $tersambung ? 'text-emerald-700' : 'text-rose-700' }}">
        ...
        <div class="card">
            ...
            <p class="mt-1 text-sm font-semibold text-slate-900 break-all">{{ $guru->whatsapp_number ?: 'nomor belum diisi' }}</p>
    ...
    <div class="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div class="flex items-baseline justify-between gap-3 border-b border-slate-200 px-5 py-3">
            <h2 class="text-sm font-semibold text-slate-900">Kelas</h2>
                    ...
                    @foreach ($kelas as $k)
                        <tr class="{{ $k->is_active ? '' : 'bg-slate-50' }}">
                            <td class="px-5 py-3 font-semibold text-slate-800">
                                {{ $k->name }}
                                @unless ($k->is_active)
                                    <span class="ml-1 rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600">diarsipkan</span>
                                @endunless
                            </td>
    ...
    <div class="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div class="flex items-baseline justify-between gap-3 border-b border-slate-200 px-5 py-3">
            <h2 class="text-sm font-semibold text-slate-900">Sesi absensi terakhir</h2>
                ...
                <table class="w-full text-left text-xs">
                    <thead class="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500">
                        <tr>
                            ...
                            <th class="px-5 py-2.5 font-semibold">Sesi</th>
                        ...
                        @foreach ($sesiTerakhir as $s)
                            <tr class="{{ $s->delivery_status === 'failed' ? 'bg-rose-50/50' : '' }}">
                                ...
                                <td class="px-5 py-2.5">
                                    <span class="rounded px-1.5 py-0.5 text-[10px] font-semibold {{ $s->status === 'submitted' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
                                        {{ $s->status }}
                                    </span>
                                </td>
                                <td class="px-5 py-2.5">
                                    @if ($s->delivery_status === 'failed')
                                        <span class="font-semibold text-rose-700">gagal</span>
    ...
    <div class="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div class="flex items-baseline justify-between gap-3 border-b border-slate-200 px-5 py-3">
            ...
            <a href="{{ route('admin.subscriptions.index') }}" class="text-[11px] font-semibold text-indigo-600 hover:text-indigo-700">Kelola di halaman langganan ›</a>
        </div>
                    ...
                    @foreach ($pembayaran as $p)
                        <tr>
                            ...
                            <td class="px-5 py-3">
                                ...
                                @endphp
                                <span class="rounded px-1.5 py-0.5 text-[10px] font-semibold {{ $gaya }}">{{ $p->status }}</span>
