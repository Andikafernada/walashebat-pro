...
<div class="space-y-8 pb-16">
        ...
        <div class="relative z-10 space-y-3">
            ...
            <div class="pt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
                <div class="flex items-center gap-1.5">
                    ...
                    @if ($otomasiAktif)
                        <span class="font-semibold text-emerald-400 uppercase tracking-wider bg-emerald-500/20 px-2 py-0.5 rounded-full border border-emerald-500/30">
                            {{ strtoupper($user->subscription_tier) }}
                        </span>
                    @else
                        <span class="font-semibold text-rose-300 uppercase tracking-wider bg-rose-500/20 px-2 py-0.5 rounded-full border border-rose-500/30">
                            Berakhir
                        </span>
                    @endif
                </div>
            ...
            @if($pendingProof)
                <div class="mt-3 rounded bg-amber-500/20 border border-amber-400/40 p-3 text-xs text-amber-200">
                    <span class="font-semibold block text-white">⏳ Konfirmasi Pembayaran Diproses</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="rounded-lg border-2 border-indigo-600 bg-white p-6 space-y-4 relative flex flex-col justify-between">
            ...
            <div class="space-y-3">
                <span class="inline-block rounded-full bg-indigo-50 px-3 py-1 text-[10px] font-semibold text-indigo-700 uppercase tracking-wider">Akses Penuh 1 Bulan</span>
                ...
                <ul class="space-y-2.5 text-xs text-slate-700 pt-3 border-t border-slate-200 font-medium">
        ...
        <div class="card space-y-4">
            <div class="border-b border-slate-200 pb-3 flex items-center justify-between">
                <div>
                    <h3 class="text-base font-semibold text-slate-900">Pembayaran Resmi DANA</h3>
            ...
            <form method="POST" action="{{ route('subscription.upload') }}" enctype="multipart/form-data" class="space-y-3 text-xs">
                ...
                <div>
                    <label class="block font-semibold text-slate-700 uppercase tracking-wider mb-1">Paket Dibatalkan / Dipilih</label>
                    <input type="text" readonly value="PRO 1 Bulan — {{ $rpHarga }}" class="h-9 w-full rounded border border-slate-200 bg-slate-100 px-3 text-xs font-semibold text-slate-800">
                </div>

                <div>
                    <label class="block font-semibold text-slate-700 uppercase tracking-wider mb-1">Nama Akun / Pengirim DANA *</label>
                    <input type="text" name="sender_name" value="{{ old('sender_name') }}" required placeholder="cth: Budi Santoso" class="form-input form-input--sm">
                </div>

                <div>
                    <label class="block font-semibold text-slate-700 uppercase tracking-wider mb-1">Metode Pembayaran</label>
                    <input type="text" readonly name="bank_name" value="DANA (083817203455)" class="h-9 w-full rounded border border-slate-200 bg-slate-100 px-3 text-xs font-semibold text-sky-700">
                </div>

                <div>
                    <label class="block font-semibold text-slate-700 uppercase tracking-wider mb-1">Foto / Tangkapan Layar Bukti DANA *</label>
