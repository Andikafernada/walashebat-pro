    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Libur Sekolah Anda</span>
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Libur Nasional</span>
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
            <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Hari Libur</span>
        ...
        <div class="space-y-4 lg:col-span-2">
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
                ...
                <div x-show="tab === 'mine'">
                            ...
                            @foreach ($mine as $h)
                                <div class="flex items-center justify-between gap-4 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs hover:border-indigo-200 hover:shadow-md transition-all">
                                    <div class="flex items-center gap-3.5">
                                        ...
                                        <div>
                                            <h4 class="font-bold text-sm text-slate-900">{{ $h->description }}</h4>
                    ...
                    @else
                        <div class="my-10 text-center">
                            ...
                            <p class="text-sm font-bold text-slate-800">Belum Ada Libur Sekolah</p>
                ...
                <div x-show="tab === 'national'" x-cloak>
                    @if ($national->isNotEmpty())
                        <div class="space-y-3">
                            @foreach ($national as $h)
                                <div class="flex items-center justify-between gap-4 rounded-2xl border border-slate-200/80 bg-slate-50/50 p-4 shadow-xs">
                                    <div class="flex items-center gap-3.5">
                                        <div class="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl bg-emerald-100 text-emerald-800 font-bold border border-emerald-200 shadow-xs">
                                            <span class="text-xs leading-none">{{ $h->start_date->format('d') }}</span>
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <h4 class="font-bold text-sm text-slate-900">{{ $h->description }}</h4>
                                                <span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-800">Nasional</span>
                                            </div>
                    ...
                    @else
                        <div class="my-10 text-center">
                            <p class="text-sm font-bold text-slate-800">Belum Ada Daftar Libur Nasional</p>
                        </div>
                    @endif
                </div>
        ...
        <div class="space-y-4">
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm space-y-4">
                ...
                <div class="flex items-center gap-3 border-b border-slate-100 pb-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">Tambah Libur Sekolah</h3>
                ...
                <form method="POST" action="{{ route('holidays.store') }}" class="space-y-4" x-data="{ start: '', end: '', loading: false }" @submit="loading = true">
                    ...
                    <div>
                        <label for="start_date" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tanggal Mulai</label>
                    ...
                    <div>
                        <label for="end_date" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tanggal Selesai</label>
                    ...
                    <div>
                        <label for="description" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Keterangan Libur</label>
