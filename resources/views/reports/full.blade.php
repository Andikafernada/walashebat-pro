    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-5 space-y-4">
        ...
        <form method="GET" x-data="{ mode: '{{ $periode['mode'] ?? 'bulan' }}' }" class="space-y-4">
            <div class="flex flex-wrap items-end gap-3">
                <div>
                    <label for="mode" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Jenis Periode</label>
                    <select id="mode" name="mode" x-model="mode" class="h-9 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-medium text-slate-800 focus:border-indigo-500 focus:outline-none">
                ...
                <div x-show="mode === 'bulan'">
                    <label for="bulan" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Pilih Bulan</label>
                    <input type="month" id="bulan" name="bulan" value="{{ isset($periode['bulan']) ? $periode['bulan']->format('Y-m') : date('Y-m') }}"
                           class="h-9 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-800 focus:border-indigo-500 focus:outline-none">
                </div>

                <template x-if="mode === 'semester'">
                    <div class="flex items-end gap-2">
                        <div>
                            <label for="sub_periode" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Periode Evaluasi Semester</label>
                            <select id="sub_periode" name="sub_periode" class="h-9 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-semibold text-slate-800 focus:border-indigo-500">
                        ...
                        <div>
                            <label for="tahun" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tahun TA</label>
                            <input type="number" id="tahun" name="tahun" min="2000" max="2100" value="{{ $periode['tahun'] ?? date('Y') }}"
                                   class="h-9 w-24 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-800 focus:border-indigo-500">
                        </div>
                    </div>
                </template>
                ...
                <template x-if="mode === 'rentang'">
                    <div class="flex items-end gap-2">
                        <div>
                            <label for="start_date" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Dari Tanggal</label>
                            <input type="date" id="start_date" name="start_date" value="{{ isset($periode['awal']) ? $periode['awal']->format('Y-m-d') : date('Y-m-01') }}"
                                   class="h-9 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-800">
                        </div>
                        <div>
                            <label for="end_date" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Sampai Tanggal</label>
            ...
            <div class="pt-3 border-t border-slate-100 space-y-2">
                ...
                --}}
                <span class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Bagian Yang Ditampilkan:</span>
