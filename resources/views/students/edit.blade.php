    ...
    <div class="max-w-3xl pb-12">
        <div class="bg-white rounded-2xl border border-emerald-200 shadow-xs p-6">
            ...
            <div class="mt-8 border-t border-rose-100 pt-6">
                <h4 class="text-xs font-bold text-rose-700 mb-3">Zona Berbahaya</h4>
                <form method="POST"
                      action="{{ route('classes.students.destroy', [$classroom, $student]) }}"
                      class="inline"
                      onsubmit="return confirm('Hapus {{ $student->name }} dari kelas ini?')">
                    @csrf @method('DELETE')
                    <button type="submit" class="inline-flex items-center justify-center rounded-xl border border-rose-200 bg-white px-4 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 transition-colors">Hapus Siswa
                    </button>
                </form>
            </div>
        </div>
