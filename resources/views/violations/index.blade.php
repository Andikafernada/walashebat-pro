        ...
        <div class="space-y-4 lg:col-span-2">
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm space-y-4">
                ...
                <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div>
                        <h3 class="text-base font-bold text-slate-900">Riwayat Catatan Pelanggaran</h3>
                        ...
                        @foreach ($violations as $v)
                            <div class="flex items-center justify-between gap-4 py-3 hover:bg-slate-50/80 px-2 rounded-xl transition-colors">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-rose-100 text-rose-800 font-bold text-xs border border-rose-200 shadow-xs">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-sm text-slate-900">{{ $v->student->name ?? 'Siswa' }}</h4>
                ...
                @else
                    <div class="my-10 text-center">
                        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-500 mb-3">
                        ...
                        <p class="text-sm font-bold text-slate-800">Belum Ada Pelanggaran Catatan</p>
        ...
        <div class="space-y-4">
            <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm space-y-4">
                ...
                <div class="flex items-center gap-3 border-b border-slate-100 pb-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">Catat Pelanggaran</h3>
                ...
                <form method="POST" action="{{ route('classes.violations.store', $class) }}" class="space-y-4" x-data="{ loading: false }" @submit="loading = true">
                    ...
                    <div>
                        <label for="student_id" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Siswa</label>
                    ...
                    <div>
                        <label for="violation_type_id" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Jenis Pelanggaran</label>
                        <select id="violation_type_id" name="violation_type_id" class="h-9 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-medium text-slate-800 focus:border-indigo-500 focus:outline-none"
                    ...
                    <div id="blok-poin-custom">
                        <label for="points" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Poin</label>
                        <input type="number" id="points" name="points" value="{{ old('points') }}" step="1" min="-100" max="100" placeholder="cth: -5 untuk mengurangi poin"
                               class="h-9 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-800 focus:border-indigo-500 focus:outline-none">
                    ...
                    <div>
                        <label for="occurred_on" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tanggal Kejadian</label>
                        <input type="date" id="occurred_on" name="occurred_on" value="{{ date('Y-m-d') }}" required class="h-9 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-800 focus:border-indigo-500 focus:outline-none">
                    </div>

                    <div>
                        <label for="note" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Catatan Tambahan <span class="text-slate-400 font-normal lowercase">(opsional)</span></label>
                        <textarea id="note" name="note" rows="2" placeholder="Tuliskan keterangan detail kejadian..." class="w-full rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-xs text-slate-800 focus:border-indigo-500 focus:outline-none"></textarea>
                    </div>
